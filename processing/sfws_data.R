...
raw_data <- read_excel("./sfws_action_plan_2022/raw_data/SFWS Action Plan October 2022 Update.xlsx")
...
transformed_df <- raw_data %>% 
  rename(title = ap_title,
         ...
         hic_ = hic_class) %>% 
  select(ap_number, title, text, success, partners, achieved, hic_, update_2020, update_2021, latest_update) %>% 
